...
pub trait CircuitBuilderHeaderVerification<F: RichField + Extendable<D>, const D: usize> {
    ...
    fn verify_header_ivc<C: GenericConfig<D, F = F> + 'static>(&mut self) -> (BoolTarget, EncodedHeaderTarget, Target, VerifierCircuitTarget, ProofWithPublicInputsTarget<D>)
    where
...
impl<F: RichField + Extendable<D>, const D: usize> CircuitBuilderHeaderVerification<F, D> for CircuitBuilder<F, D> {
    ...
    C::Hasher: AlgebraicHasher<F>,
    {
        ...
        let previous_proof_elements = self.parse_public_inputs(previous_header_verification_proof_with_pis.public_inputs.clone());
    ...
    fn header_verification_ivc_common_data(&mut self) -> CommonCircuitData<F, D> {
        ...
        let k_i_fields = k_is.iter().map(|x| F::from_canonical_u64(*x)).collect::<Vec<_>>();
        ...
        let barycentric_weights_fields = barycentric_weights.iter().map(|x| F::from_noncanonical_u64(*x)).collect::<Vec<_>>();
    ...
    fn parse_public_inputs(&mut self, public_inputs: Vec<Target>) -> PublicInputsElements {
        ...
        PublicInputsElements { 
            initial_block_hash: AvailHashTarget(public_inputs_iter.by_ref().take(HASH_SIZE).collect_vec().as_slice().try_into().unwrap()),
            initial_block_num: public_inputs_iter.by_ref().take(1).collect_vec()[0],
            initial_data_root_accumulator: AvailHashTarget(public_inputs_iter.by_ref().take(HASH_SIZE).collect_vec().as_slice().try_into().unwrap()),
            latest_block_hash: AvailHashTarget(public_inputs_iter.by_ref().take(HASH_SIZE).collect_vec().as_slice().try_into().unwrap()),
            latest_block_num:  public_inputs_iter.by_ref().take(1).collect_vec()[0],
            latest_data_root_accumulator: AvailHashTarget(public_inputs_iter.take(HASH_SIZE).by_ref().collect_vec().as_slice().try_into().unwrap()),
        }
    }
...
mod tests {
    use anyhow::Result;
    use log::Level;
    use plonky2::{plonk::{config::{PoseidonGoldilocksConfig, GenericConfig}, circuit_data::CircuitConfig, circuit_builder::CircuitBuilder, prover::prove}, recursion::{cyclic_recursion::check_cyclic_proof_verifier_data, dummy_circuit::cyclic_base_proof}, iop::witness::{PartialWitness, WitnessWrite}, util::timing::TimingTree, field::types::PrimeField64};
    use crate::{utils::{tests::{BLOCK_530508_HEADER, BLOCK_530509_HEADER, BLOCK_530510_HEADER, BLOCK_530508_PARENT_HASH}, WitnessEncodedHeader}, header_verification::CircuitBuilderHeaderVerification};
    ...
    fn test_verify_headers_ivc() -> Result<()> {
        ...
        initial_pi.extend(initial_block_hash_val.iter().map(|b| F::from_canonical_u64(*b as u64)));
        ...
        pw.set_proof_with_pis_target(&previous_header_verification_proof_with_pis, &proof1);
        pw.set_verifier_data_target(&verifier_data_target, &cyclic_circuit_data.verifier_only);
        ...
        pw.set_proof_with_pis_target(&previous_header_verification_proof_with_pis, &proof2);
        pw.set_verifier_data_target(&verifier_data_target, &cyclic_circuit_data.verifier_only);
