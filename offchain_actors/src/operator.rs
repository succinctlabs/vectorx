...
async fn submit_proof_gen_request(
    ...
    let mut add_nine = diff.into_iter().take(9).cloned().collect::<Vec<_>>();
    ...
    'main_loop: loop {
        select! {
            ...
            justification = fused_justification_sub.next() => {
                ...
                if justification_to_process.is_none() && unwrapped_just.commit.target_number >= last_processed_block_num.unwrap() + 20 {
