...
fn get_type_size<'b>(
    md: &Metadata,
    substrate_type: &'b UntrackedSymbol<TypeId>,
    type_chunk_sizes: &mut Vec<Chunk>,
) {
    let td = md
        .runtime_metadata()
        .types
        .resolve(substrate_type.id)
        .unwrap();
    ...
    match &td.type_def {
        ...
        TypeDef::Array(a) => {
            ...
            add_constant_size(
                array_len * extract!(Chunk::ConstantSize(_), array_element_size[0]).unwrap(),
                type_chunk_sizes,
            );
        }
        ...
        _ => {
            println!("Unhandled type {:?}", td.type_def)
        }
    }
}
    ...
    for p in pallets.iter() {
        ...
        if p.event.is_some() {
            let events = md
                .runtime_metadata()
                .types
                .resolve(p.event.as_ref().unwrap().ty.id);
...
{CONSTRUCTOR_CODE}
                ...
                match chunk {
                    ...
                    Chunk::Sequence(sequence_chunks) => {
                        ...
                        for (j, sequence_chunk) in sequence_chunks.iter().enumerate() {
                            constructor_code.push_str(
                                format!(
                                    "        event_chunks[{}][{}][{}].sequence_chunks.push();\n",
                                    pallet_idx, event_idx, i
                                )
                                .as_str(),
                            );
                            match sequence_chunk {
                                ...
                                Chunk::Sequence(inner_sequence_chunks) => {
                                    ...
                                    for (k, inner_sequence_chunk) in
                                        inner_sequence_chunks.iter().enumerate()
                                    {
                                        ...
                                        match inner_sequence_chunk {
                                            ...
                                            Chunk::Sequence(_) => {
                                                panic!(
                                                    "Nested sequence of depth 2 is not supported"
                                                );
                                            }
                                        }
                                    }
    ...
    println!(
        "contract_template: {}",
        contract_template.replace("{CONSTRUCTOR_CODE}", constructor_code.as_str())
    );
}
...
async fn test_nested_sequence() {
    ...
    let pallet_idx = 20;
    let event_idx = 2;
    let event_data: [u8; 98] = [
        4, 84, 86, 92, 68, 217, 86, 27, 84, 33, 157, 68, 32, 5, 81, 171, 48, 223, 54, 187, 202, 12,
        215, 119, 153, 24, 104, 224, 147, 68, 199, 15, 117, 27, 149, 141, 163, 204, 231, 204, 4, 2,
        103, 1, 23, 142, 125, 100, 45, 45, 27, 139, 81, 54, 4, 164, 206, 101, 103, 123, 218, 213,
        223, 55, 70, 58, 88, 59, 44, 12, 216, 111, 45, 155, 160, 151, 213, 227, 52, 12, 46, 12, 15,
        87, 245, 255, 17, 27, 7, 16, 63, 159, 186, 177, 121, 176, 48, 1,
    ];
